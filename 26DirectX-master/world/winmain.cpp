...
class TriangleRenderer : public Component { // 화면에 삼각형을 그리는 렌더링 컴포넌트입니다.
public:
    float colorR, colorG, colorB; // 이 컴포넌트가 그릴 삼각형의 고유 색상 값(RGB)을 저장합니다.

    TriangleRenderer(float r, float g, float b) : colorR(r), colorG(g), colorB(b) {} // 생성자에서 색상을 초기화 리스트로 받습니다.

    void Start() override { // 초기화 함수
        printf("[%s] TriangleRenderer 장착 완료!\n", pOwner->name.c_str()); // 렌더러가 붙었음을 콘솔에 출력합니다.
    }
    ...
    void Render() override { // 렌더링 단계에서 호출됩니다.
        // 1. 현재 GameObject의 위치와 고유 색상을 상수 버퍼 구조체에 담습니다.
        CBData cb; // 셰이더로 보낼 데이터를 담을 임시 구조체를 생성합니다.
        cb.offsetX = pOwner->x; // 주인이 되는 게임 오브젝트의 현재 X 좌표를 넣습니다.
        cb.offsetY = pOwner->y; // 주인이 되는 게임 오브젝트의 현재 Y 좌표를 넣습니다.
        cb.pad1 = cb.pad2 = 0.0f; // 16바이트 정렬을 위해 패딩 변수를 0으로 채웁니다.
        cb.r = colorR; cb.g = colorG; cb.b = colorB; cb.a = 1.0f; // 초기화때 설정한 고유 색상(RGB)과 불투명도(A)를 넣습니다.

        // 2. CPU에서 만든 cb 데이터를 GPU 메모리(상수 버퍼)로 복사하여 전송합니다.
        g_pImmediateContext->UpdateSubresource(g_pConstantBuffer, 0, nullptr, &cb, 0, 0);

        // 3. 갱신된 상수 버퍼를 정점 셰이더(VS)의 0번 슬롯(b0)에 바인딩합니다.
        g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pConstantBuffer);

        // 4. 세팅이 완료되었으므로 정점 3개를 그려서 삼각형 하나를 출력하도록 GPU에 명령을 내립니다.
...
public:
    ...
    void Initialize(HWND hWnd) { // 엔진 초기화 및 DirectX 리소스를 생성하는 함수입니다.
        ...
        // --- 정점 버퍼 생성 (하얀색의 기본 삼각형 하나를 정의) ---
        Vertex vertices[] = {
            {  0.0f,  0.1f, 0.5f, 1.0f, 1.0f, 1.0f, 1.0f }, // 위쪽 뾰족한 정점 (X, Y, Z, R, G, B, A)
            {  0.1f, -0.1f, 0.5f, 1.0f, 1.0f, 1.0f, 1.0f }, // 오른쪽 아래 정점
            { -0.1f, -0.1f, 0.5f, 1.0f, 1.0f, 1.0f, 1.0f }, // 왼쪽 아래 정점
        };
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // [Player 1] 요구사항에 맞춰 객체 조립 (우측 배치, 방향키 이동, 파란색 삼각형)
    ...
    // [Player 2] 요구사항에 맞춰 객체 조립 (좌측 배치, WASD 이동, 붉은색 삼각형)
